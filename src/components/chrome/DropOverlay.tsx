 ...
 * While a folder drop handler is registered (First run, game not found) it asks for the game folder instead.
 */
export function DropOverlay() {
  const { t } = useTranslation();
  const active = useUi((s) => s.dragActive);
  const folder = useUi((s) => s.folderDrop !== null);
      ...
      {active && (
        <div className="pointer-events-none fixed inset-2 z-[60] flex items-center justify-center rounded-dialog border-2 border-dashed border-amber bg-bg-scrim">
          <div className="flex flex-col items-center gap-1.5">
            <span className="text-title text-ink-1">
              {folder ? t('firstRun.notFound.dropOverlay') : t('common.drop.title')}
            </span>
            <span className="font-mono text-mono-sm text-ink-3">
              {folder ? t('firstRun.notFound.dropFormats') : t('common.drop.formats')}
            </span>
          </div>
        </div>
      )}
